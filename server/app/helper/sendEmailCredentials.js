...
const sendEmailCredentials = async (user, password) => {
  try {
    ...
    const info = await transporter.sendMail({
      ...
      to: user.email,
      subject: "Your Login Credentials - Swiggy (Food Delivery App)",
      text: `Welcome to  Swiggy (Food Delivery App)\n\nLogin Credentials:\nName: ${user.name}\nRole: ${user.role}\nEmail: ${user.email}\nPassword: ${password}\n\nPlease login and change your password immediately.`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; padding: 20px; border: 1px solid #ddd; border-radius: 10px;">
          <h2 style="color: #e56d00;">Welcome to Swiggy (Food Delivery App), ${user.name}!</h2>
          ...
          <p style="font-size: 12px; color: #888;">This email was sent to ${user.email} as part of your registration on Swiggy (Food Delivery App).</p>
        </div>
      `,
    });
