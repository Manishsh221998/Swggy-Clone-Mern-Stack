...
const sendOtpEmail = async (user) => {
  try {
    ...
    const subject = "Your OTP Code - Swiggy (Food Delivery App)";
    const html = `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f9f9f9; padding: 30px;">
        <div style="max-width: 600px; margin: auto; background: #ffffff; padding: 30px; border-radius: 10px; box-shadow: 0 2px 6px rgba(0,0,0,0.05);">
          <div style="text-align: center;">
            <h1 style="color: #fc8019; margin-bottom: 10px;">Swiggy</h1>
            <h3 style="color: #333;">OTP Verification</h3>
          </div>
          <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;">
          <p style="font-size: 16px; color: #555;">Hi <strong>${user.name}</strong>,</p>
          <p style="font-size: 15px; color: #555;">
            To proceed with your account verification on <strong>Swiggy (Food Delivery App)</strong>, please use the following One-Time Password (OTP):
          </p>
          ...
          <p style="font-size: 12px; color: #aaa; text-align: center;">
            You received this email because your email address is registered with Swiggy. If you did not request this OTP, please ignore this email.
          </p>
          <p style="text-align: center; font-size: 12px; color: #aaa;">&copy; ${new Date().getFullYear()} Swiggy (Food Delivery App)</p>
        </div>
      </div>
    `;
